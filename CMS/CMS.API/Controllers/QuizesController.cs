...
namespace CMS.API.Controllers
{
    ...
    public class QuizesController : ControllerBase
    {
        ...
        public async Task<ActionResult> RequestReviews(QuizReviewRequestDTO model)
        {
            var result = await _quizesService.QuizReviewRequest(model);
            if (result.StatusCode == 200)
            {
                return Ok(result);
            }
        ...
        [HttpGet("summarized/{courseId}/{userId}")]
        public async Task<ActionResult> Summarized(string courseId, string userId)
        {
            var summary = await _quizesService.Summary(courseId, userId);
