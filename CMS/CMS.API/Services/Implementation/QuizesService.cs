...
namespace CMS.API.Services
{
    public class QuizesService : IQuizesService
    {
        ...
        public async Task<QuizResponseDto<QuizReviewRequest>> QuizReviewRequest(QuizReviewRequestDTO model)
        {
            try
            {
                return await _quizesRepo.QuizReviewRequest( model);
            }
            catch (Exception ex)
            {
                var response = new QuizResponseDto<QuizReviewRequest>();
                response.StatusCode = 500;
                response.ErrorMessages = new List<string> { "Unable to review user total score" };
                response.DisplayMessage = "Error";
                return response;
            }
        }
        ...
        public async Task<QuizResponseDto<QuizSummaryDto>> Summary(string courseId,string userId)
        {
            try
            {
                return await _quizesRepo.Summary(courseId, userId);
            ...
            catch (Exception ex)
            {
                ...
                response.StatusCode = 500;
                response.ErrorMessages = new List<string> { "Unable to  fetch summary" };
                response.DisplayMessage = "Error";
                return response;
            }
        }
